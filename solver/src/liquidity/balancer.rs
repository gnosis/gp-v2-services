...
mod tests {
    ...
    use model::TokenPair;
    use num::BigRational;
    use shared::sources::balancer::pool_fetching::{BalancerPool, BalancerPoolState, TokenState};
    use shared::{
        dummy_contract,
        sources::balancer::pool_fetching::{
            MockBalancerPoolFetching, WeightedPool, WeightedPoolState,
        },
    };
    ...
    async fn fetches_liquidity() {
        ...
        let pools = vec![
            BalancerPool::Weighted(WeightedPool {
                ...
                reserves: hashmap! {
                    H160([0x70; 20]) => WeightedPoolState {
                    ...
                    H160([0x71; 20]) => WeightedPoolState {
                        token_state: TokenState {
                    ...
                    H160([0xb0; 20]) => WeightedPoolState {
                        token_state: TokenState {
            ...
            BalancerPool::Weighted(WeightedPool {
                ...
                reserves: hashmap! {
                    H160([0x73; 20]) => WeightedPoolState {
                        token_state: TokenState {
                            balance: 1_000_000_000_000_000_000u128.into(),
                        scaling_exponent: 0,
                        },
