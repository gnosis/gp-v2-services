...
use ethcontract::{H160, U256, Account};
...
pub fn create(
    ...
    solvers
        ...
        .map(|solver_type| match solver_type {
            ...
            SolverType::Baseline => boxed(BaselineSolver::new(account.clone(), base_tokens.clone())),
            ...
            SolverType::Matcha => {
                let matcha_solver =
                    MatchaSolver::new(account.clone(), web3.clone(), settlement_contract.clone(), chain_id).unwrap();
